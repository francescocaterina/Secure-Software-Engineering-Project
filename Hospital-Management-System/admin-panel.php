...
if(isset($_POST['app-submit']))
{
  ...
  if(date("Y-m-d",$appdate1)>=$cur_date){
    if((date("Y-m-d",$appdate1)==$cur_date and date("H:i:s",$apptime1)>$cur_time) or date("Y-m-d",$appdate1)>$cur_date) {
      ...
      // FIX: Prepared Statement per il controllo dell'orario
      $check_stmt = $con->prepare("SELECT apptime FROM appointmenttb WHERE doctor=? AND appdate=? AND apptime=?");
      $check_stmt->bind_param("sss", $doctor, $appdate, $apptime);
      $check_stmt->execute();
      $check_stmt->store_result();

      if($check_stmt->num_rows == 0){
          // FIX: Prepared Statement per l'inserimento dell'appuntamento
          ...
          $stmt->bind_param("isssssssss", $pid, $fname, $lname, $gender, $email, $contact, $doctor, $docFees, $appdate, $apptime);
...
{
    // FIX SQL Injection: Prepared Statement per l'aggiornamento dello stato utente
    $stmt_cancel = $con->prepare("UPDATE appointmenttb SET userStatus='0' WHERE ID = ?");
...
function generate_bill(){
  ...
  // FIX SQL Injection: Migrazione a query parametrizzata con segnaposto
  $stmt_bill = $con->prepare("SELECT p.pid, p.ID, p.fname, p.lname, p.doctor, p.appdate, p.apptime, p.disease, p.allergy, p.prescription, a.docFees FROM prestb p INNER JOIN appointmenttb a ON p.ID=a.ID AND p.pid = ? AND p.ID = ?");
  $stmt_bill->bind_param("si", $pid, $_GET['ID']);
  $stmt_bill->execute();
  $query_res = $stmt_bill->get_result();
...
<html lang="en">
  ...
  <body style="padding-top:50px;">
   ...
   <div class="container-fluid" style="margin-top:50px;">
    <h3 style = "margin-left: 40%; padding-bottom: 20px; font-family: 'IBM Plex Sans', sans-serif;"> Welcome &nbsp<?php echo htmlspecialchars($username, ENT_QUOTES, 'UTF-8'); ?></h3>
    <div class="row">
