...
if(isset($_GET['cancel'])) {
    // 1. Validazione RIGOROSA: Controlliamo se l'ID è esclusivamente un numero intero
    if(filter_var($_GET['ID'], FILTER_VALIDATE_INT) === false) {
        // 2. Se contiene payload malevoli o caratteri strani, BLOCCIAMO l'operazione
        echo "<script>alert('Attacco rilevato: Input non valido. Operazione bloccata!'); window.location.href = 'doctor-panel.php';</script>";
    } else {
        // 3. Se l'input è pulito, procediamo con i Prepared Statements per sicurezza assoluta
        $id_appuntamento = $_GET['ID'];
        $stmt = mysqli_prepare($con, "UPDATE appointmenttb SET doctorStatus='0' WHERE ID = ?");

        // 'i' indica che il parametro passato DEVE essere un intero (integer)
        mysqli_stmt_bind_param($stmt, "i", $id_appuntamento);
...
.text-primary {
  ...
  <div class="col-md-8" style="margin-top: 3%;">
    ...
    // FIX SQL Injection: Utilizzo del Prepared Statement
    ...
    while ($row = $result->fetch_assoc()){
        // FIX XSS: Output encoding applicato a ogni variabile proveniente dal database
